...
class EmprestimosView(QWidget):
    ...
    def emprestar(self):
        ...
        if disponibilidade:
            id_usuario = self.selecao_usuario.currentData()
            
            id_livro = self.selecao_livro.currentData()
            ...
            if ok:
                ...
                valor = -1
                self.livro_controller.atualizar_quantidade_disponivel(id_livro, valor)
                self.carregar()
                self.limpar_campos()
                self.atualizar_botoes()
            else:
